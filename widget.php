...
Description: Diese Plugin erm&ouml;glicht das Einbinden des eVendi.de-Widget 
Version: 1.01
Author: eVendi GmbH & Co. KG
Author URI: https://www.evendi.de/
*/
$eVWidgetPluginVersion = '1.01';
function addEvendiWidgetToPost($content='') {
	try {
...
function addEvendiWidgetFooter() {
	$Opt = eVendiLoadOptions();  
	...
	echo '</script><script type="text/javascript" src="//data.evendi.de/v101/liveprice.js"></script><script type="text/javascript">';
	echo 'var _eVWiget = new _eVWigetLoader();_eVWiget.load();</script>';
}

function eVendiLoadOptions() {
	return Array (
		'PubTag' => get_option('_eVWidgetOption_PubTag', 'evwpwidem-de'),
		'Font' => get_option('_eVWidgetOption_Font', 'Arial, Verdana'),
		'FSize' => get_option('_eVWidgetOption_FontSize', '14'),
		'CSpl' => get_option('_eVWidgetOption_CSpLine', '#CCCCCC'),
		'CLnk' => get_option('_eVWidgetOption_CLnk', '#000000'),
		'CBg' => get_option('_eVWidgetOption_CBg', '#FFFFFF'),
		'OffCnt' => get_option('_eVWidgetOption_OfferCnt', '7')
	);  
}

function eVendiAdminOptions() {
	if(!current_user_can('manage_options')){wp_die( __('You do not have sufficient permissions to access this page.') );}
	$hidden_field_name='eVendiWigUpdate';
	...
	if(isset($_POST[$hidden_field_name]) && $_POST[$hidden_field_name]=='Y'){
		...
		update_option('_eVWidgetOption_PubTag', $Opt['PubTag']);
		update_option('_eVWidgetOption_Font', $Opt['Font']);
		update_option('_eVWidgetOption_FontSize', $Opt['FSize']);
		update_option('_eVWidgetOption_CSpLine', $Opt['CSpl']);
		update_option('_eVWidgetOption_CBg', $Opt['CBg']);
		update_option('_eVWidgetOption_CLnk', $Opt['CLnk']);
		update_option('_eVWidgetOption_OfferCnt', $Opt['OffCnt']);
	...
	<h2>eVendi.de WordPress Plugin (Version <?php  echo $GLOBALS['eVWidgetPluginVersion']; ?>)</h2>
		<?php 
		...
		<form name="form1" method="post" action="" style="margin:0px">
			...
			<p><table cellpadding="0" cellspacing="5" border="0" style="table-layout:fixed;">
				<tr>
					...
					<td>Der „PublisherTag“ (PubTag) ist für die Abrechnung und die Zuordnung der generierten Klicks/Weiterleitungen notwendig. Diesen erhalten Sie <u>nach</u> erfolgreicher Registrierung bei eVendi.de. Für die Dauer des kostenfreien Testzeitraums ist eine Demo PubTag voreingestellt. Die Dauer des Tests ist nicht begrenzt. Auszahlungen von Provisionen jedoch sind ohne einen von eVendi.de zugewiesenen PubTag und ohne vorherige Registrierung nicht möglich.</td>
				</tr>
				<tr>
      ...
      <p>Über eVendi.de Datenbank werden dann im fertig erstellten Beitrag automatisch die entsprechenden Angebote angezeigt. Dabei passt sich die Anzeige und Darstellung des eVendi.de Widgets automatisch der verfügbaren Breite an.</p>
...
function addEvendiToAdminMen() { 
	add_options_page('eVendi.de-Widget', 'eVendi.de-Widget', 8, __FILE__, 'eVendiAdminOptions');
}

add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'addEvendiPagePluginSettings', 10, 2);
add_action('wp_footer', 'addEvendiWidgetFooter');
add_filter('the_content', 'addEvendiWidgetToPost');
add_action('admin_menu', 'addEvendiToAdminMen');
?>